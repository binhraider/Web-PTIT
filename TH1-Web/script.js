...
function submitQuizz() {
  ...
  const textAnswerCount = getTextAnswerCount();
...
function getTextAnswerCount() {
  const textQuestions = quizzData.textQuestions;
  let count = 0;
  for (let i = 0; i < textQuestions.length; i++) {
    // Kiểm tra xem câu trả lời có được nhập không
    const textAnswer = document.getElementById(`text-${i}`).value.trim();
    if (textAnswer !== '') {
      count++;
    }
  }
